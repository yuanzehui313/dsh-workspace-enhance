 ...
 * plugin (no `cordis` import) so it composes in profiles whose node_modules
 * tree (e.g. the pnpm-managed tui profile) does not contain cordis.
 *
 ...
 * browser UI) are not extensible from an additive plugin. This entry registers
 * slash commands so terminal clients (dsh-tui) can drive the overlay's recycle
 * bin, cross-workspace move, and file preview features without the web sidebar.
 */
export function apply(ctx) {
  ...
  const handlers = new WorkspaceEnhanceHandlers(ctx);
  ctx.effect(function* () {
    yield ctx.commands.register({
      name: "trash",
      description: "回收站：列出已删除的会话与工作区",
      handler: (invocation) => handlers.trash(invocation)
    });
    yield ctx.commands.register({
      name: "trash-restore",
      description: "从回收站恢复：/trash-restore <sessionId | workspaceId>",
      handler: (invocation) => handlers.trashRestore(invocation)
    });
    yield ctx.commands.register({
      name: "trash-purge",
      description: "彻底删除回收站条目：/trash-purge <sessionId | workspaceId>",
      handler: (invocation) => handlers.trashPurge(invocation)
    });
    yield ctx.commands.register({
      name: "move",
      description: "移动会话到其他工作区：/move <sessionId> <workspaceId | ungrouped>",
      handler: (invocation) => handlers.move(invocation)
    });
    yield ctx.commands.register({
      name: "preview",
      description: "预览工作区内的文件：/preview <path>",
      handler: (invocation) => handlers.preview(invocation)
    });
  ...
  }, "workspaceEnhance commands");
}

class WorkspaceEnhanceHandlers {
  constructor(ctx) {
    this.ctx = ctx;
  }
