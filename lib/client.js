window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function CostMeterWidget() {
			...
			// 面板绝对定位于 chip 下方，展开不撑宽容器 → chip 不位移、点击位置不变
			const vw = window.innerWidth || 1200;
			const vh = window.innerHeight || 800;
			const alignRight = !pos || pos.x > vw / 2;
			const flipUp = !!pos && pos.y > vh / 2;
			...
			const panel = react.createElement(
				"div",
				{
					...
					style: {
						...
						position: "absolute",
						top: flipUp ? "auto" : "calc(100% + 6px)",
						bottom: flipUp ? "calc(100% + 6px)" : "auto",
				...
				failed
					...
					: react.createElement(
						...
						}, "双击面板收起 · 官方价(高峰/空闲两档)见 ~/.dsh/cost-meter/config.json · 60s 自动刷新")
					)
			);
