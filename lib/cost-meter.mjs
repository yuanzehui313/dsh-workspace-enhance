 ...
 *   recomputed when the day changes, the pricing schema version changes, the
 *   on-disk session log set changes, or the live session set changes (a
 *   deletion invalidates the cache even though the log files are untouched).
...
/** Fingerprint of the live session set; null when the DB is unavailable. */
function liveSessionFingerprint() {
  const live = liveSessionIds();
  if (!live) return null;
  let h = 0x811c9dc5;
  for (const id of [...live.raw].sort()) {
    for (let i = 0; i < id.length; i += 1) {
      h ^= id.charCodeAt(i);
      h = Math.imul(h, 0x01000193) >>> 0;
    }
  }
  return `${live.raw.size}:${h.toString(16)}`;
}

/** Aggregate fingerprint of the on-disk session logs (count / bytes / newest mtime). */
function diskFingerprint() {
  ...
  const walk = (dir) => {
    ...
    for (const entry of entries) {
      ...
      if (!entry.name.endsWith(".jsonl.zstd") && !entry.name.endsWith(".jsonl")) continue;
      try {
  ...
  walk(SESSIONS_ROOT);
  return `${count}:${bytes}:${Math.trunc(maxMtime)}`;
}
...
export function computeWeek() {
  ...
  const liveIds = liveSessionIds(); // null → DB unavailable, count everything

  for (const file of files) {
    // 软删除的会话：DB 里已无此 id，日志虽在磁盘上但不再计费
    if (liveIds && !liveIds.has(file.sessionId)) continue;
    const lines = decodeSessionLog(file.path).split("\n").filter(Boolean);
...
/** Cached week summary; recomputes when the day, the pricing schema, the on-disk logs, or the live session set changed. */
export function getWeekSummary() {
  const now = new Date();
  const todayLabel = localDateLabel(now);
  if (!existsSync(META_ROOT)) mkdirSync(META_ROOT, { recursive: true });
  const cachePath = join(META_ROOT, "week.json");
  const diskFp = diskFingerprint();
  const liveFp = liveSessionFingerprint(); // null → DB unavailable, trust cache
  let cached = null;
  try {
  ...
  if (
    cached?.version === PRICING_SCHEMA &&
    cached?.date === todayLabel &&
    Array.isArray(cached.days) && cached.days.length === WEEK_DAYS &&
    cached?.diskFingerprint === diskFp &&
    (liveFp === null || cached?.liveFingerprint === liveFp)
  ) {
    return cached.days;
  }
