...
7. **会话合并已移除**：新会话已自带整个工作区上下文，合并会话不再需要；相关按钮与入口全部删除（仅残留无引用的 i18n 文案）。
8. **会话行图标**：分叉 / 归档 / 删除直接图标 + 分叉树形渲染；重命名图标从 ⋯ 菜单提出，
   会话行 ⋯ 菜单已移除。
9. **工作区行图标**：重命名 / 新建会话 / 删除 直接平铺。（「管理文件夹」图标已移除，
   其入口改为文件夹区 / 文件区标题行右侧的 + 按钮。）
...
21. **文件夹 Git 面板**：文件夹区中 git 仓库文件夹行显示当前分支 chip 与三色状态点
    （🟡 黄 = 有未提交修改 / 🔵 蓝 = 落后远程 / 🟢 绿 = 与远程一致）；点击 chip 弹出 Git
    菜单——切换分支、新建分支、更新代码（`git pull --ff-only`）、推送代码（自动暂存全部
    改动 → AI 生成中文提交信息 → 提交 → 推送，结果回显菜单）。所有操作直接作用于该
    文件夹的磁盘工作区。

## 文件夹 Git 面板（改造后）

![工作区侧边栏 Git 面板](docs/workspace-git-panel.png)

文件夹区中，git 仓库文件夹行右侧显示当前分支 chip 与状态圆点：

- 🟡 **黄** = 有未提交的修改
- 🔵 **蓝** = 落后远程（有未更新）
- 🟢 **绿** = 与远程一致

点击分支 chip 弹出 Git 菜单：**切换分支**（点击列表行）、**新建分支**（输入框 + 回车）、
**更新代码**（`git pull --ff-only`）、**推送代码**（`git add -A` → AI 生成中文 Conventional
Commits 提交信息 → 提交 → 推送，结果回显在菜单中）。每次操作后分支信息与目录树自动
刷新。
