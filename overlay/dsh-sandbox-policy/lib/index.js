...
function renderPolicyContext(policy) {
	switch (policy.mode) {
		...
		case "workspace-write": return `Current DSH file policy: workspace-write. Any available operation enforced by the DSH file sandbox may modify files under the session workspace roots: ${JSON.stringify(policy.workspaceRoots ?? [policy.workspaceRoot])}. Some platform temporary areas may also be writable.`;
	...
	*/
	resolve(request = {}) {
		...
		if (session !== void 0) {
			...
			if (registry !== void 0 && typeof registry.findBySessionId === "function") {
				const workspace = registry.findBySessionId(session.id);
				if (workspace !== void 0 && Array.isArray(workspace.paths)) {
