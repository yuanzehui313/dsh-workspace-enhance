...
async function serveStatic(pathname, res, distRoot, distIndex, renderIndex) {
	...
	const serveIndex = async () => {
		...
		res.writeHead(200, {
			...
			// The index carries the client-module boot graph (window.__DSH_BOOT__);
			// no-cache keeps plugin-set changes visible on a plain refresh instead
			// of serving a stale graph from browser heuristics.
