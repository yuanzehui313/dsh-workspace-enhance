...
/** The ordered workspace roots of one policy: the multi-root list when present, else the single root. */
function policyRoots(policy) {
	const roots = policy.workspaceRoots;
	return roots !== void 0 && Array.isArray(roots) && roots.length > 0 ? roots : [policy.workspaceRoot];
}
...
function bwrapProfileArgs(policy) {
	...
	if (policy.mode === "workspace-write") {
		args.push("--tmpfs", "/tmp");
		for (const root of policyRoots(policy)) args.push("--bind", root, root);
	}
	return args;
}
/**
...
function landlockProfileArgs(policy) {
	...
	if (policy.mode === "workspace-write") readWrite.push("/tmp", ...policyRoots(policy));
	return grantArgs({
		readOnly: ["/"],
		readWrite
	});
...
var LocalSandboxProvider = class extends SandboxProvider {
	...
	*/
	materializeAclGrant(sessionId, workspaceRoots) {
