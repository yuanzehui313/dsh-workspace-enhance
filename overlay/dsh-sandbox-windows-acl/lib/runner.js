...
async function main() {
	...
	const seamManaged = parsed.writeSids.length > 0 || parsed.tempWriteSid !== void 0;
	if (parsed.mode === "read-only" && seamManaged) fail("read-only does not accept --write-sid or --temp-write-sid");
	if (parsed.mode === "workspace-write" && parsed.writeSids.length === 0 !== (parsed.tempWriteSid === void 0)) fail("workspace-write requires --write-sid and --temp-write-sid together");
	if (parsed.mode === "workspace-write") for (const workspace of parsed.workspaces) assertTempRootOutsideWorkspace(workspace, parsed.temp);
	...
	try {
		...
		let writeSid;
		let additionalWriteSids = [];
		let privateTempSid;
		if (parsed.mode === "workspace-write") {
			writeSid = workspaceWriteSid(parsed.workspaces[0]);
			if (seamManaged) {
				if (parsed.writeSids.length !== parsed.workspaces.length) fail("--write-sid count must match --workspace count");
				for (let slot = 0; slot < parsed.workspaces.length; slot++) {
					if (parsed.writeSids[slot] !== workspaceWriteSid(parsed.workspaces[slot])) fail("--write-sid does not match --workspace");
				}
			...
			additionalWriteSids = parsed.writeSids.slice(1);
		}
		sandbox = new AclSandbox({
			...
			...additionalWriteSids.length > 0 ? { additionalWriteSids } : {},
