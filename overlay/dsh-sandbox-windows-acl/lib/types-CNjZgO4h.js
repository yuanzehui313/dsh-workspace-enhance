...
var AclSandbox = class {
	...
	writeSidPtr;
	tempWriteSidPtr;
	/** Additional workspace capability SIDs whose ACEs extend the write allowlist (multi-root workspaces). */
	additionalWriteSidPtrs = [];
	...
	constructor(options) {
		...
		this.additionalWriteSids = options.additionalWriteSids ?? [];
		...
		for (const extra of this.additionalWriteSids) {
			if (extra === this.writeSid) throw new Error("AclSandbox additional workspace write SIDs must differ from the workspace write SID");
			if (extra === this.tempWriteSid) throw new Error("AclSandbox additional workspace write SIDs must differ from the temp write SID");
		}
	}
	...
	async init() {
		...
		try {
			...
			this.additionalWriteSidPtrs = this.additionalWriteSids.map(parseSid);
			...
			restrictedToken = createRestrictedToken(api, currentToken, logonSid, [this.writeSidPtr, ...this.additionalWriteSidPtrs, this.tempWriteSidPtr].filter((sid) => sid !== void 0), { world: worldSid }, this.mode);
			...
			for (const [label, sidPtr] of [["workspace write SID", this.writeSidPtr], ["temp write SID", this.tempWriteSidPtr], ...this.additionalWriteSidPtrs.map((ptr, slot) => [`additional workspace write SID ${slot}`, ptr])]) freeSidBestEffort(api, sidPtr, label, cleanupFailures);
	...
	*/
	dispose() {
		...
		for (const [label, sidPtr] of [["workspace write SID", this.writeSidPtr], ["temp write SID", this.tempWriteSidPtr], ...this.additionalWriteSidPtrs.map((ptr, slot) => [`additional workspace write SID ${slot}`, ptr])]) freeSidBestEffort(api, sidPtr, label, failures);
		const token = this.token;
