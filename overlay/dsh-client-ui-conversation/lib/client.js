		...
		*/
		...
		function browserDraftAttachment(file) {
			...
			return {
				...
				previewUrl: image ? URL.createObjectURL(file) : textAttachmentPreview(file),
				file
			};
		}
		/** Image types the image pipeline admits; every other file is a document attachment. */
		const IMAGE_ATTACHMENT_TYPES = new Set(["image/png", "image/jpeg", "image/webp", "image/gif"]);
		/** Whether a pasted/dropped file rides the image pipeline (content uploaded). */
		function isImageFile(file) {
			return IMAGE_ATTACHMENT_TYPES.has(file.type);
		}
		/** Text-attachment MIME types serialized with their content into the prompt (html/md/txt). */
		const TEXT_ATTACHMENT_TYPES = new Set(["text/html", "text/markdown", "text/plain"]);
		/** Whether a pasted/dropped file is a readable text attachment (MIME or extension). */
		function isTextAttachment(file) {
			if (TEXT_ATTACHMENT_TYPES.has(file.type)) return true;
			return /\.(html?|md|markdown|txt)$/i.test(file.name ?? "");
		}
		/** Escape one string for embedding as an SVG text node. */
		function svgText(value) {
			return value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;");
		}
		/** SVG thumbnail chip showing the file's extension badge and its name (truncated). */
		function textAttachmentPreview(file) {
			const name = file.name || "file";
			const ext = (name.split(".").pop() || "file").toLowerCase();
			const badge = ext.length <= 4 ? ext.toUpperCase() : "FILE";
			const shown = name.length > 14 ? name.slice(0, 13) + "\u2026" : name;
			const svg = '<svg xmlns="http://www.w3.org/2000/svg" width="112" height="96"><rect width="112" height="96" rx="12" fill="#eef1f6"/><text x="56" y="38" font-family="Arial, sans-serif" font-size="16" font-weight="bold" fill="#3b5bdb" text-anchor="middle">' + svgText(badge) + '</text><text x="56" y="64" font-family="Arial, sans-serif" font-size="11" fill="#495057" text-anchor="middle">' + svgText(shown) + '</text></svg>';
...
;
		...
		var ConversationController = class extends _deepseek_ai_cordis.Service {
			...
			blocks;
			/** Workspace runtime (list + manager) for recording pasted text attachments. */
			workspaces;
			...
			*/
			async sendSession(session, text, imageIds, mode) {
				...
				if (!result.ok) throw new Error(`conversation.send failed: ${result.error.code}: ${result.error.message}`);
				this.registerWorkspaceFiles(session, attachments);
				this.releaseDraftImages(attachments);
			}
			/**
			* Record pasted text attachments (html/md/txt) as workspace files mounted
			* on the owning session, rendered above that session in the workspace tree.
			* Purely best-effort: failures must never fail the send itself.
			* @param session - the session the attachments were sent into.
			* @param attachments - the draft attachments just serialized.
			*/
			registerWorkspaceFiles(session, attachments) {
				const workspaces = this.workspaces;
				if (workspaces === void 0) return;
				const docs = attachments.filter((attachment) => attachment.kind === "file");
				if (docs.length === 0) return;
				const view = workspaces.list.getSnapshot().items.find((workspace) => workspace.sessionIds.includes(session.sessionId));
				if (view === void 0) return;
				const run = async () => {
					const entries = [];
					for (const attachment of docs) {
						if (attachment.file.size > 5242880) {
							console.warn("session file upload skipped (over 5MB):", attachment.file.name);
							continue;
						}
						const bytes = new Uint8Array(await attachment.file.arrayBuffer());
						let binary = "";
						for (let offset = 0; offset < bytes.length; offset += 32768) binary += String.fromCharCode(...bytes.subarray(offset, offset + 32768));
						entries.push({ name: attachment.file.name, data: btoa(binary), sessionId: session.sessionId });
					}
					if (entries.length > 0) await workspaces.manager.importFiles(view.workspaceId, entries);
				};
				run().catch(() => {});
			}
			/**
			...
			*/
			createDraftImages(files) {
				for (const file of files) if (isImageFile(file)) imageMediaType(file.type);
			...
			serializeImages(images) {
				return Promise.all(images.map(async (file) => {
					if (isTextAttachment(file)) {
						...
						return { type: "text", text: "[附件 " + (file.name || "pasted.txt") + "]\n" + text };
					}
					...
					return { type: "text", text: "[附件 " + (file.name || "pasted-file") + "]" };
				}));
			}
		};
		...
		* (running/removed/promptError) are self-selected via useSession. */
		...
		function InputBar({ useSession, useInput, inputActions, keyboard, addImages, removeImage, draftImages, resolveSubmitMode, toggleCommandMenu, stop, command, t, renderSlot, useNotices, useLexicon, useMenuLauncher, useProjection, sessionId, variant, disabled: inert = false, blocked, workspacePickerOpen = false, onRequestWorkspace, placeholder, accessory, overlay, leftItems, rightItems, footer }) {
			...
			const intakeImages = (0, react.useCallback)((files) => {
				...
				const rejected = (() => {
					if (imageLimits !== void 0) {
						...
						const contentFiles = files.filter((file) => isImageFile(file) || isTextAttachment(file));
						if (imageFiles.some((file) => !imageLimits.mediaTypes.includes(file.type))) return addImages(files);
						...
						if (contentFiles.some((file) => file.size > imageLimits.maxImageBytes)) return t("image.fileTooLarge", { size: imageSizeText(imageLimits.maxImageBytes) });
						if (attachments.reduce((sum, attachment) => sum + attachment.file.size, 0) + imageFiles.reduce((sum, file) => sum + file.size, 0) > imageLimits.maxMessageImageBytes) return t("image.totalTooLarge", { size: imageSizeText(imageLimits.maxMessageImageBytes) });
					}
			...
			const railItems = (0, react.useMemo)(() => attachments.map((attachment) => ({
				...
				removeLabel: t(attachment.kind === "file" ? "image.removeDocument" : "image.remove", { name: attachment.file.name }),
				attachment
			})), [attachments, t]);
			const onSelect = (e) => {
		...
		*/
		...
		const zh = {
			...
			"image.dropBlocked": "当前无法添加图片或文件",
			"image.pending": "待发送附件",
			"image.openOriginal": "查看原图",
			"image.openOriginalLabel": "{label}，点击查看原图",
			"image.remove": "移除图片 {name}", "image.removeDocument": "移除附件 {name}",
			...
			"image.fileTooLarge": "单个附件不能超过 {size}",
		...
		const en = {
			...
			"image.dropBlocked": "Images or files cannot be added right now",
			"image.pending": "Pending attachments",
			"image.openOriginal": "View original",
			"image.openOriginalLabel": "{label}, click to view original",
			"image.remove": "Remove image {name}", "image.removeDocument": "Remove attachment {name}",
			...
			"image.fileTooLarge": "Each attachment must be smaller than {size}",
