	...
	*/
	...
	get sessionIds() {
		return this.record.sessionIds.filter((id) => {
			const moved = this.host.movedWorkspaceOf(id);
			if (moved !== void 0) return moved === this.id;
			return this.host.sessionPath(id) === this.record.path;
		});
	}
	...
	/**
	* Replace the workspace's additional roots. Every candidate is canonicalized
	* and must exist as a directory; the primary path and duplicates are dropped.
	* An empty array clears every extra root.
	* @param paths - Additional directory paths in any spelling.
	*/
	async setAdditionalPaths(paths) {
		const seen = new Set([this.record.path]);
		...
		for (const path of paths) {
			...
			try {
				...
				throw new Error(`cannot add workspace path '${path}': it does not resolve`, { cause: error });
			}
			if (!(await stat(canonical)).isDirectory()) throw new Error(`cannot add workspace path '${path}': it is not a directory`);
			if (seen.has(canonical)) continue;
			seen.add(canonical);
			additionalPaths.push(canonical);
		}
		await this.mutate((record) => sameSessionIds(record.additionalPaths ?? [], additionalPaths) ? record : {
			...record,
			additionalPaths
		});
	}
		...
		try {
			next = await this.host.table().update(this.id, (current) => {
				...
				const sessionIds = changed.sessionIds.filter((id) => {
					const moved = this.host.movedWorkspaceOf(id);
					if (moved !== void 0) return moved === this.id;
					return this.host.sessionPath(id) === changed.path;
				});
...
const workspaceDomainState = z.object({
	...
	movedSessionWorkspaces: z.record(z.string(), z.string()).default({}),
...
var WorkspaceRegistry = class extends Service {
	...
	host = {
		...
		movedWorkspaceOf: (id) => this.requireState().movedSessionWorkspaces[id]
	};
	constructor(ctx) {
	...
	*/
	async create(path, title, additionalPaths = []) {
		...
		for (const extra of additionalPaths) {
			const resolved = await realpathNormalize(extra);
			if (!(await stat(resolved)).isDirectory()) throw new Error(`cannot create a workspace at '${canonical}': additional path '${extra}' is not a directory`);
			if (!seen.has(resolved)) {
				seen.add(resolved);
				extras.push(resolved);
			}
		}
		return await this.enqueueOperation(() => this.createCanonical(canonical, title, extras));
	}
	/**
	...
	*/
	...
	/**
	* Look up the workspace that accounts a session id.
	* @param sessionId - Session id to resolve.
	* @returns the owning workspace entity, or `undefined` when none accounts it.
	*/
	findBySessionId(sessionId) {
		for (const entity of this.entities.values()) if (entity.record.sessionIds.includes(sessionId)) return entity;
	}
	/**
	...
	*/
	...
	/**
	* Move one session into another workspace, or out of every workspace when
	* `workspaceId` is null/undefined (ungrouped). Workspace membership is
	* durable and survives the cwd-based pruning of ordinary mutations; the
	* session's own cwd and log location stay untouched.
	* @param sessionId - The session to move.
	* @param workspaceId - Target workspace id, or null/undefined to ungroup.
	* @returns resolution after durability.
	*/
	moveSession(sessionId, workspaceId) {
		return this.enqueueOperation(async () => {
			const source = this.findBySessionId(sessionId);
			const target = workspaceId == null ? void 0 : this.get(workspaceId);
			if (workspaceId != null && target === void 0) throw new Error(`cannot move session '${sessionId}': target workspace '${workspaceId}' not found`);
			if (source !== void 0 && target?.id !== source.id) await source.detachSession(sessionId);
			if (target === void 0 || target.id === source?.id) {
				const state = this.requireState();
				if (state.movedSessionWorkspaces[sessionId] === void 0) return;
				const moved = {
					...state.movedSessionWorkspaces
				};
				delete moved[sessionId];
				await this.setState({
					...state,
					movedSessionWorkspaces: moved
				});
				return;
			}
			const state = this.requireState();
	...
	* visibility everywhere. An unarchived id resolves without writing.
	...
	*/
	...
	/**
	* Soft-delete one session into the recycle bin: its workspace accounting and
	* persisted data stay untouched, so restoring is a marker removal.
	...
	*/
	...
	/**
	* Restore one soft-deleted session from the recycle bin.
	...
	*/
	...
	/**
	* Purge one soft-deleted session from every durable set (the physical
	* removal of its data is the caller's persistence concern).
	...
	*/
	...
	/**
	* Soft-delete one workspace registration into the recycle bin (the record
	* and its sessions stay intact).
	* @param id - The workspace to move to the recycle bin.
	*/
	softDeleteWorkspace(id) {
		return this.enqueueOperation(async () => {
			const state = this.requireState();
			if (state.deletedWorkspaceIds.includes(id)) return;
			await this.setState({
				...state,
				deletedWorkspaceIds: [...state.deletedWorkspaceIds, id]
			});
		});
	}
	/**
	* Restore one soft-deleted workspace registration.
	* @param id - The workspace to restore.
	*/
	restoreWorkspace(id) {
		return this.enqueueOperation(async () => {
			const state = this.requireState();
	...
	/**
	* Purge one soft-deleted workspace: the durable record and order entry are
	* removed (sessions and session logs are kept).
	* @param id - The workspace to purge.
	...
	*/
	purgeWorkspace(id) {
		return this.enqueueOperation(async () => {
			...
			return await this.deleteKnown(id);
		});
	}
	/** The recycle-bin session id set. */
	get deletedSessionIds() {
		return this.requireState().deletedSessionIds;
	}
	/** The recycle-bin workspace id set. */
	get deletedWorkspaceIds() {
	...
	*/
	...
	async createCanonical(canonical, title, additionalPaths = []) {
		for (const entity of this.entities.values()) if (entity.path === canonical) return entity;
		...
		try {
			await this.setState({
				...
				movedSessionWorkspaces: state.movedSessionWorkspaces
			});
		} catch (error) {
			this.entities.delete(id);
			try {
	...
	async deleteKnown(id) {
		...
		const nextState = {
			...
			deletedSessionIds: state.deletedSessionIds,
			deletedWorkspaceIds: state.deletedWorkspaceIds,
			movedSessionWorkspaces: Object.fromEntries(Object.entries(state.movedSessionWorkspaces).filter(([, workspaceId]) => workspaceId !== id))
		};
		await this.setState({
			...nextState,
			pendingMutation: {
	...
	*/
	...
	validateStoredState(state) {
		...
		for (const [id, record] of table.entries()) {
			...
			for (const additional of record.additionalPaths ?? []) {
				if (additional === record.path) throw new Error(`workspace domain is inconsistent: workspace '${id}' lists its own primary path '${additional}' as additional`);
				if (seenAdditional.has(additional)) throw new Error(`workspace domain is inconsistent: workspace '${id}' repeats additional path '${additional}'`);
				seenAdditional.add(additional);
			}
