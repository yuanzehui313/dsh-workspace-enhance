...
var PersistenceCoordinator = class {
	...
	*/
	...
	/** Session ids removed via {@link remove}; later appends for them are swallowed. */
	...
	*/
	...
	async appendCore(id, events) {
		assertSupportedEvents(events, id);
		if (events.length === 0 || this.removedIds.has(id)) return;
	...
	*/
	...
	/**
	* Delete one session's stored artifact and drop its in-memory bookkeeping.
	* @param id - persisted session to remove.
	* @param signal - optional cancellation for queued and backend work.
	* @returns true when a stored artifact was removed, false when none existed.
	*/
	remove(id, signal) {
		const retired = Promise.resolve(this.retirements.get(id));
		return (signal === void 0 ? retired : observeQueuedAbort(retired, signal, () => false)).then(() => this.serialize(id, () => this.removeCore(id, signal), signal));
	}
	async removeCore(id, signal) {
		signal?.throwIfAborted();
		const removed = await this.backend.removeStored(id, signal);
		this.states.delete(id);
		this.chains.delete(id);
		this.retirements.delete(id);
		/* A live session may still flush buffered events after removal; the
		marker swallows those appends so the deleted artifact cannot resurrect
		through write-behind re-adoption. */
