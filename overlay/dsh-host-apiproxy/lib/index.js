...
/** session.merge request payload: at least two source sessions merged into one new session. */
const sessionMergeRequestSchema = z$1.object({
	workspaceId: workspaceIdSchema,
	sessionIds: z$1.array(sessionIdSchema).min(2),
	agentPreset: z$1.string().optional()
});
/** session.merge response value: the new session carrying the merged context. */
const sessionMergeValueSchema = z$1.object({ sessionId: sessionIdSchema });
...
const sessionRestoreRequestSchema = z$1.object({ sessionId: sessionIdSchema });
/** session.restore response value. */
const sessionRestoreValueSchema = z$1.object({ restored: z$1.literal(true) });
/** session.purge request payload. */
const sessionPurgeRequestSchema = z$1.object({ sessionId: sessionIdSchema });
/** session.purge response value. */
const sessionPurgeValueSchema = z$1.object({ deleted: z$1.literal(true) });
...
*/
/**
* Usable fallback project directory for workspace-less sessions: a
* filesystem root (e.g. `C:\` — the working directory of a
* Task-Scheduler-launched server) is never a writable cwd, so root-level
* defaults resolve to the home directory instead.
*/
function normalizeDefaultCwd(cwd) {
	if (cwd === void 0 || cwd === "") return homedir();
	try {
		const root = parse(cwd).root;
		if (root !== "" && root === cwd) return homedir();
	} catch {}
	return cwd;
}
function createApiProxy(ctx, defaults) {
	...
	*/
	...
	function ensureWorkspace(path, additionalPaths) {
		const operation = workspaceCreationChain.then(async () => {
			const existing = await ctx.workspaceRegistry.resolveByPath(path);
			if (existing !== void 0) {
				if (additionalPaths !== void 0 && additionalPaths.length > 0) await existing.setAdditionalPaths(additionalPaths);
				return {
					workspace: existing,
					created: false
				};
			...
			return {
				workspace: await ctx.workspaceRegistry.create(path, void 0, additionalPaths ?? []),
				created: true
			};
		});
	...
	/** Render one message content block into the merge transcript. */
	function renderMergeBlock(block) {
		switch (block.type) {
			case "text": return block.text;
			case "tool-call": return `[工具调用 ${block.name}]`;
			case "tool-result": return `[工具结果] ${(block.content ?? []).map((part) => part.type === "text" ? part.text : "").filter(Boolean).join("")}`;
			case "image": return "[图片]";
			default: return "";
		}
	}
	/** Render one derived message for the merge transcript. */
	function renderMergeMessage(message) {
		const role = message.role === "user" ? "用户" : "助手";
		const body = (message.content ?? []).map(renderMergeBlock).filter((text) => text !== "").join("\n");
		return `### ${role}\n${body === "" ? "(空)" : body}`;
	}
	/**
	* The model-visible messages of one session: live sessions use their derived
	* surface; cold sessions fold the persisted events with the same per-node
	* rule (last version of each message id wins).
	*/
	async function mergeMessagesFor(sessionId) {
		const live = ctx.agents.get(sessionId);
		if (live !== void 0 && typeof live.session?.deriveMessages === "function") return live.session.deriveMessages();
		const source = await historySourceFor(sessionId);
		const cut = historyCutOf(source, true);
		...
		for (const event of cut.events) {
			...
			if (message === void 0 || message.content.length === 0) continue;
	...
	*/
	async function listVisibleSessionSummaries(signal, includeDeleted = false) {
		signal?.throwIfAborted();
		...
		const deleted = new Set(ctx.workspaceRegistry.deletedSessionIds);
		return items.filter((item) => !deleted.has(item.sessionId));
	}
	/**
	...
	*/
	...
	return {
		sessions: {
			...
			async merge(request) {
				const { workspaceId, sessionIds, agentPreset } = request.payload;
				const workspace = ctx.workspaceRegistry.get(WorkspaceId(workspaceId));
				if (workspace === void 0) return workspaceNotFound(request, workspaceId);
				const unique = [...new Set(sessionIds)];
				if (unique.length < 2) return err(request, {
					code: "merge-needs-sessions",
					message: "select at least two distinct sessions to merge",
					details: {}
				});
				/* Expand each selected session to its full fork subtree so the merged
				context covers every session nested under the tree nodes. */
				...
				if (persistence !== void 0 && typeof persistence.list === "function") {
					...
					for (const header of await persistence.list()) {
						...
						const bucket = childrenByParent.get(header.parentSession);
						if (bucket === void 0) childrenByParent.set(header.parentSession, [header.id]);
				...
				const mergeIds = [...expanded];
				const mergeId = `session-${randomUUID()}`;
				try {
					await ensureSession(mergeId, workspace.path, false, agentPreset);
				} catch (error) {
					return err(request, {
						code: "internal",
						message: `failed to create the merged session: ${String(error)}`,
						details: {}
					});
				}
				try {
					await workspace.attachSession(mergeId);
				} catch (error) {
					return err(request, {
						code: "workspace-attach-failed",
						message: `merged session "${mergeId}" was created but could not attach to workspace "${workspaceId}": ${String(error)}`,
						details: {
							sessionId: mergeId,
							workspaceId
						}
				...
				const agent = ctx.agents.get(mergeId);
				if (agent === void 0) return err(request, {
					code: "internal",
					message: `merged session "${mergeId}" has no live agent`,
					details: {}
				});
				const sections = [];
				for (let index = 0; index < mergeIds.length; index++) {
					const sessionId = mergeIds[index];
					let messages;
					try {
						messages = await mergeMessagesFor(sessionId);
					} catch (error) {
						return err(request, {
							code: "session-not-found",
							message: `cannot merge session "${sessionId}": ${error instanceof Error ? error.message : String(error)}`,
							details: { sessionId }
						});
					}
					sections.push(`## 会话 ${index + 1}: ${sessionId}`);
					for (const message of messages) sections.push(renderMergeMessage(message));
				}
				const transcript = `# 合并会话上下文\n\n以下内容由 ${mergeIds.length} 个历史会话合并而成，作为本会话的参考上下文：\n\n${sections.join("\n\n")}`;
				const mergedMessage = createUserMessage({
					content: [{
						type: "text",
						text: transcript
					}],
				...
				agent.session.append("user/message", mergedMessage, { surfaceOp: "append" });
				return ok(request, {
					sessionId: mergeId
				});
			},
			async delete(request) {
				...
				/* Soft delete: the session moves to the recycle bin; its persisted
				data and workspace accounting stay intact for restoration. */
				const live = ctx.agents.get(sessionId);
				if (live !== void 0) try {
					live.cancel({ kind: "user" }, { keepInbox: true });
				} catch {
					/* a settled agent rejects cancel; deletion proceeds */
				}
			...
			async purge(request) {
				...
				if (workspace !== void 0) try {
					...
					return err(request, {
						code: "internal",
						message: `cannot detach session "${sessionId}" from its workspace: ${String(error)}`,
						details: { sessionId }
					});
				}
				const persistence = ctx.get("sessionPersistence");
				if (persistence === void 0 || typeof persistence.remove !== "function") return err(request, {
					code: "internal",
					message: "session persistence does not support removal",
					details: { sessionId }
				});
				try {
					...
					return err(request, {
						code: "internal",
						message: `cannot purge session "${sessionId}": ${String(error)}`,
						details: { sessionId }
					});
				}
				await ctx.workspaceRegistry.purgeSession(sessionId);
				return ok(request, { deleted: true });
			},
		...
		workspace: {
			list(request) {
				return Promise.resolve(ok(request, {
					items: ctx.workspaceRegistry.list().filter((workspace) => !ctx.workspaceRegistry.deletedWorkspaceIds.includes(workspace.id)).map(workspaceView),
					archivedSessionIds: [...ctx.workspaceRegistry.archivedSessionIds]
				}));
			},
			async create(request) {
				const { path, additionalPaths } = request.payload;
				try {
					const { workspace, created } = await ensureWorkspace(path, additionalPaths);
					return ok(request, {
			...
			async setAdditionalPaths(request) {
				...
				try {
					const operation = workspaceCreationChain.then(() => workspace.setAdditionalPaths(payload.additionalPaths));
					workspaceCreationChain = operation.then(() => void 0, () => void 0);
					await operation;
					...
					return err(request, {
						code: "workspace-invalid-path",
						message: `cannot set additional paths of workspace "${payload.workspaceId}": ${error instanceof Error ? error.message : String(error)}`,
			...
			async delete(request) {
				...
				const operation = workspaceCreationChain.then(() => ctx.workspaceRegistry.softDeleteWorkspace(WorkspaceId(workspaceId)));
				workspaceCreationChain = operation.then(() => void 0, () => void 0);
				await operation;
				if (ctx.workspaceRegistry.get(WorkspaceId(workspaceId)) === void 0) return workspaceNotFound(request, workspaceId);
				return ok(request, { deleted: true });
			},
			async restore(request) {
				const { workspaceId } = request.payload;
				await ctx.workspaceRegistry.restoreWorkspace(WorkspaceId(workspaceId));
				return ok(request, { restored: true });
			},
			async purge(request) {
				const { workspaceId } = request.payload;
				const operation = workspaceCreationChain.then(() => ctx.workspaceRegistry.purgeWorkspace(WorkspaceId(workspaceId)));
				workspaceCreationChain = operation.then(() => void 0, () => void 0);
				if (!await operation) return workspaceNotFound(request, workspaceId);
				return ok(request, { deleted: true });
			},
			async trash(request, signal) {
				const deletedSessionIds = new Set(ctx.workspaceRegistry.deletedSessionIds);
				const sessions = (await listVisibleSessionSummaries(signal, true)).filter((item) => deletedSessionIds.has(item.sessionId));
				const workspaces = ctx.workspaceRegistry.list().filter((workspace) => ctx.workspaceRegistry.deletedWorkspaceIds.includes(workspace.id)).map(workspaceView);
				return ok(request, {
					sessions,
					workspaces
				});
			...
			async moveSession(request) {
				const { sessionId, workspaceId } = request.payload;
				if (workspaceId !== null && ctx.workspaceRegistry.get(WorkspaceId(workspaceId)) === void 0) return workspaceNotFound(request, workspaceId);
				await ctx.workspaceRegistry.moveSession(sessionId, workspaceId === null ? void 0 : workspaceId);
				const workspace = workspaceId === null ? void 0 : ctx.workspaceRegistry.get(WorkspaceId(workspaceId));
				return ok(request, {
					workspace: workspace === void 0 ? null : workspaceView(workspace)
				});
			},
			...
			async archiveSession(request) {
				const { sessionId } = request.payload;
				/* Archive the fork subtree BELOW the target, keeping the source
				session visible; a leaf session (no children) archives itself. */
				const descendants = [];
				const persistence = ctx.get("sessionPersistence");
				if (persistence !== void 0 && typeof persistence.list === "function") {
					const childrenByParent = /* @__PURE__ */ new Map();
				...
				const ids = descendants.length > 0 ? descendants : [sessionId];
				try {
					for (const id of ids) {
						try {
							await ctx.workspaceRegistry.archiveSession(id);
						} catch (error) {
							if (id === sessionId && error instanceof WorkspaceUnknownSessionError) throw error;
							/* descendant misses are tolerated */
						}
					}
				} catch (error) {
		...
		host: {
			...
			async listDirectory(request, signal) {
				...
				try {
					return ok(request, await capability.list(request.payload.path, signal, { includeFiles: request.payload.includeFiles === true }));
				} catch (error) {
					if (signal.aborted) return err(request, {
						code: "cancelled",
...
const hostListDirectoryRequestSchema = z$1.object({
	path: z$1.string().optional(),
	includeFiles: z$1.boolean().optional()
});
...
/** workspace.create request payload: the existing directory to adopt plus optional additional roots. */
const workspaceCreateRequestSchema = z$1.object({
	path: z$1.string(),
	additionalPaths: z$1.array(z$1.string()).optional()
});
...
const workspaceSetAdditionalPathsValueSchema = z$1.object({ workspace: workspaceViewSchema });
...
/** workspace.restore request payload. */
const workspaceRestoreRequestSchema = z$1.object({ workspaceId: workspaceIdSchema });
/** workspace.restore response value. */
const workspaceRestoreValueSchema = z$1.object({ restored: z$1.literal(true) });
/** workspace.purge request payload. */
const workspacePurgeRequestSchema = z$1.object({ workspaceId: workspaceIdSchema });
/** workspace.purge response value. */
const workspacePurgeValueSchema = z$1.object({ deleted: z$1.literal(true) });
/** workspace.trash request payload (empty object literal). */
const workspaceTrashRequestSchema = z$1.object({});
/** workspace.trash response value: the recycle-bin contents. */
const workspaceTrashValueSchema = z$1.object({
	sessions: z$1.array(sessionSummarySchema),
	workspaces: z$1.array(workspaceViewSchema)
});
/** workspace.moveSession request payload (null target ungroups the session). */
const workspaceMoveSessionRequestSchema = z$1.object({
	sessionId: sessionIdSchema,
...
const workspaceMoveSessionValueSchema = z$1.object({ workspace: workspaceViewSchema.nullable() });
//#endregion
...
const UNARY_ROUTES = {
	...
	"session.merge": {
		schema: sessionMergeRequestSchema,
		invoke: (api, r) => api.sessions.merge(r)
	},
	"session.delete": {
	...
	"session.restore": {
		schema: sessionRestoreRequestSchema,
		invoke: (api, r) => api.sessions.restore(r)
	},
	"session.purge": {
		schema: sessionPurgeRequestSchema,
		invoke: (api, r) => api.sessions.purge(r)
	},
	"session.history": {
	...
	"workspace.restore": {
		schema: workspaceRestoreRequestSchema,
		invoke: (api, r) => api.workspace.restore(r)
	},
	"workspace.purge": {
		schema: workspacePurgeRequestSchema,
		invoke: (api, r) => api.workspace.purge(r)
	},
	"workspace.trash": {
		schema: workspaceTrashRequestSchema,
		invoke: (api, r, signal) => api.workspace.trash(r, signal)
	},
	"workspace.moveSession": {
		schema: workspaceMoveSessionRequestSchema,
		invoke: (api, r) => api.workspace.moveSession(r)
	},
	"skill.list": {
...
var ApiProxyService = class extends Service {
	...
	constructor(ctx, config) {
		...
		const api = createApiProxy(ctx, {
			...
			cwd: normalizeDefaultCwd(process.cwd()),
